...
|---|---|---|---|
| 4a | `fix-dimension-constraint-name` | rename the nine check constraints whose names the schema doubled: migration 0002 passed names that already began with `ck_<table>_` and the naming convention wrapped the pattern around them again, so the database and the metadata disagree on every check constraint of every table. Alembic never reports this — it does not compare check constraints at all — while a downgrade that drops one by the declared name would fail against a real database. One reversible migration, no data touched, plus the integration guard that compares catalog names with metadata names. | medium |
