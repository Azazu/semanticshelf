...
- [x] 3.2 Query validation: required, non-empty after trimming, at most 256 characters **measured after trimming** (`normalised_query` in the service, so the bound is not a constraint on the raw parameter — FR-TXT-2 bounds the trimmed query), with a far larger bound on the raw parameter so padding cannot make the request unbounded (NFR-SEC-5); 422 problem details naming the parameter otherwise. Verify: unit tests for trimming, for the empty cases and for both sides of the bound; api tests for a missing query, an empty one, one of only spaces, one a character too long, one whose padding carries the raw string past the bound while its trimmed length fits (which a `max_length` on the parameter refuses — demonstrated), and one that exceeds the raw guard.
...
- [x] 3.8 Every status this operation answers is in the OpenAPI document: the 503 of a build without the search model is declared on the route, since the factory declares only the application-wide 422 and 500 (FR-OPS-4, design decision 2). Verify: an api test reads the operation's responses and asserts the 503 is there with the problem media type.
