...
- Executing a job reads the stored original, embeds it with the model the job
  names, and writes the vector **in the same transaction that finishes the
  job**. A repeat of a delivery is harmless: the write is an upsert on
  `(asset_id, model)`.
...
- A job whose asset has been deleted finishes as `failed` with
  `asset-deleted`, without a retry.
...
- Unchanged: the schema. Change 3 created `indexing_jobs` with the two indexes
  the claim and the status query need; no migration is written.
