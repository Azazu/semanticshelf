...
- [x] 1.1 Add two dependency groups: `ui` (Streamlit) and `screenshots`
  ...
  either.
- [x] 1.2 Settle the multipage entry point against the installed Streamlit
...
- [x] 1.3 `tests/ui/test_layering_ui.py`: nothing under `ui/` reaches `app.`.
...
- [x] 2.1 `ui/client.py`: the base URL from `API_BASE_URL` with the documented
  ...
  method.
- [x] 2.2 Every refusal becomes `ServiceError(title, detail)` built from the
...
- [x] 2.3 A picture's address is built by joining the base URL with the
...
- [x] 3.1 **Search**: a description, a grid of thumbnails with scores, the
...
- [x] 3.2 **Browse**: a paginated grid, the tag cloud from `/tags` as a filter,
  ...
  asked for.
- [x] 3.3 **Upload**: a file, tags and metadata, and the service's answer shown
  ...
  detail. `AppTest` cannot drive a file uploader, so the widget is replaced with
  one that has already been used and everything after it is the page's own path
  — which is what the test needed to exercise.
- [x] 3.4 **Status**: `/ready`, the counts per model and state, the bytes the
...
- [x] 4.1 `make ui` runs the application; `make test-ui` runs the tests of the
...
- [x] 4.2 `pytest.ini` markers and `make check` stay honest: the `ui` tests are
...
- [x] 5.1 `scripts/screenshots.py`: start the API and the UI on ports it picked,
...
- [x] 5.2 The script leaves nothing running: both children are started in their
...
- [x] 6.1 `docs/how-to/demo-ui.md`: what each page is for, how to run it
...
- [x] 6.2 `AGENTS.md`'s layout line for `ui/` matches what was built, and
  `docs/reference/commands.md` lists the three new make targets. Verify: both
  read as the repository is.

## 7. Evidence

- [x] 7.1 A real run end to end: `make demo`, `make run`, `make ui`, each page
...
- [x] 7.2 `make check` and `make test-integration` green the way CI runs them
