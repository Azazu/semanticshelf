...
- [x] 9.3 Acceptance on GitHub (the user pushes the branch): both jobs green. Verify: the user reports the run.
- [x] 9.4 `openspec validate add-background-indexing --strict` and `scripts/pregate-verify.sh gate2 add-background-indexing` pass; request Gate 2 with `/gate-review add-background-indexing 2`.
