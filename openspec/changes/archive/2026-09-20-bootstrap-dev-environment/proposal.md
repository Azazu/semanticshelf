...
4. **Makefile guard.** While `alembic.ini` is absent (the application scaffold of change 2 brings Alembic and `app/main.py` together), `make migrate`, `make revision`, `make run` and `make test-integration` print `[SKIP] no alembic.ini — application not scaffolded yet` and exit 0, as a Make-level conditional (`ifeq ($(wildcard alembic.ini),)`), never silently. `make check` (lock check, lint, format check, types, unit tests) is real from this change on; the new first step `make lock-check` (`uv lock --check`) catches a `uv.lock` that drifted from `pyproject.toml`, which the CI command `uv sync --frozen` does not (design decision 9; added during apply with the user's approval on 2026-09-20).
5. **CI.** The `python` job activates by construction once `pyproject.toml` exists; its `make migrate && make test-integration` step becomes the loud SKIP until change 2, so the job is green now and needs no edit for change 2. `.github/workflows/ci.yml` is not edited by this change; if the branch run proves an edit necessary, the change stops (`blocked` handoff), the edit is designed and tasked here, and Gate 1 is requested again before it is made. The workflow file is linted locally with a pinned `actionlint` image before the push.
...
- No edit to `.github/workflows/ci.yml` (neither job) and no change to the verifier scripts. A workflow edit that a CI run proves necessary is out of this change's Gate 1 approval: it re-enters through an updated design, new tasks and a new Gate 1 round.

## Impact

- New: `pyproject.toml`, `.python-version`, `uv.lock`, `LICENSE`, `app/__init__.py`, `tests/unit/test_package.py`.
- `Makefile` (guard on `migrate`, `revision`, `run`, `test-integration`; new `lock-check` target as the first step of `check`), `docs/how-to/local-development.md`, `docs/reference/commands.md`, `README.md` (licence line).
- `.github/workflows/ci.yml`: not edited; the `python` job activates through the existing `detect` probe. A run that proves an edit necessary blocks the change until a new Gate 1 round approves the designed edit (design decision 9, task 5.5).
