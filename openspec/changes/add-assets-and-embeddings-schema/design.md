...
9. **Reversibility is an integration test on a throwaway database**, not a separate CI job. The test creates a database of its own under a name unique to the invocation (a fixed name would let a run drop a database it did not create, and two runs would drop each other's — Gate 2 round 1, finding 1), migrates to head, fingerprints the schema from the catalog (tables, columns, types, nullability, defaults, constraints, index definitions, sorted), migrates down to base and up again, fingerprints once more, compares, and drops the database. This satisfies NFR-REL-3 with no new command, no `pg_dump` dependency and no CI edit, and it runs everywhere the integration suite runs. The specification's wording is amended to match. What it does not prove: that a downgrade preserves data, which no downgrade in this project claims.
