...
- [ ] 4.5 mypy strict and ruff lint: a temporary `app/_probe.py` with an unannotated function and an unused import makes `make types` and `make lint` fail; remove it. Verify: both outputs recorded.
- [ ] 4.6 Format check: a temporary `app/_fmt_probe.py` that is valid Python but not formatted (`x=1` without spaces, single-quoted strings) passes `make lint` and makes `make fmt-check` fail with the file named; remove it. Verify: both outputs recorded, `git status --short` clean.
...
- [ ] 6.1 Commits are made after tasks 3–4 have run against the uncommitted files, so every body carries the evidence of the probes aimed at its own diff: `chore(uv): add the uv project and tool configuration` (tasks 1.1–1.2: `pyproject.toml`, `.python-version`, `uv.lock`, `LICENSE`, `app/__init__.py`; body: 3.1, 3.2, 4.4, 4.5, 4.6 outputs), `test: add the package smoke test` (task 1.3: `tests/unit/test_package.py`; body: 4.1 and 4.3 outputs), `chore(make): skip application targets until the scaffold exists` (task 2.1: `Makefile`; body: the four SKIP outputs and 4.2), `docs: describe the bootstrap tooling` (task 5.1–5.2). No agent trailer anywhere. Verify: `git log --format=%B main..HEAD | grep -c '^Co-Authored-By'` prints `0` and `git log --oneline main..HEAD` shows the four commits (plus review/handoff commits).
