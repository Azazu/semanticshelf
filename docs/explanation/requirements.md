...
- **FR-AST-10** Listing: `GET /api/v1/assets` with `limit` (default 20, max 100), `offset` (max 10 000), `tags_all`, `tags_any` (FR-FLT-2), `meta.<key>` (FR-FLT-3, since change 12 — the same conditions a search takes, refused the same way), `source`, `index_status=<model>:<status>`; ordered by `created_at` desc, `id` desc; the response carries `items`, `limit`, `offset`, `has_more` (computed by fetching `limit + 1`). No `total` (§4, stability rules).
...
- **FR-IDX-2** Stage 1 runner: after the transaction commits, a FastAPI `BackgroundTasks` callback drains pending jobs (bounded batch) in the API process through the same claim/execute/finish functions the worker later uses. Stage 3 worker: `semanticshelf worker [--once] [--batch N]` runs a loop that claims jobs with `SELECT … FOR UPDATE SKIP LOCKED` (batch `WORKER_BATCH_SIZE`, default 4), extracts, upserts and finishes, and when nothing is due waits `WORKER_POLL_SECONDS` (default 2) before looking again; several worker processes MAY run concurrently without coordination. What that gives is **at-least-once execution with a fenced completion**, not exactly-once: a lease that expires under a runner which is alive, slow or paused is claimed by another, both may compute the same vector, and what makes that harmless is the lease check on every finish plus the upsert on `(asset_id, model)` (FR-IDX-3). When the worker is configured (`INDEXING_RUNNER=worker`, added in change 13) nothing else executes work: the API enqueues only, and so do the commands of FR-CLI-1 that otherwise finish what they imported. The HTTP contract is identical under both runners.
- **FR-IDX-3** Job lifecycle: `pending` → `running` (claimed; `started_at`, `attempts + 1`, `lease_expires_at = now + JOB_LEASE_SECONDS`, default 600) → `done` (`finished_at`) or `failed` (`last_error`, ≤ 2 KB, never a stack trace, never file contents: the exception's class always, and its message only when the service raised that exception itself and therefore wrote the message — for a third-party failure the class alone, because nothing can tell which parts of a library's message came from the picture it was reading; corrected in change 6, Gate 1). A `running` job whose lease expired is reclaimable (the runner crashed): claiming treats `running AND lease_expires_at < now()` like `pending`. Delivery is at-least-once; the embedding write is an upsert on `(asset_id, model)`, so a repeated execution is harmless.
...
- **FR-TXT-1** `GET /api/v1/search/text?q=<text>&limit&offset&min_score&tags_all&tags_any&meta.<key>` embeds `q` with the search model's text tower — `clip-vit-l14` unless `model` names another (FR-IMG-4) — and returns the nearest assets by cosine similarity of that model's image embeddings.
...
- **FR-IMG-1** `POST /api/v1/search/image` (multipart `file` plus the query fields of FR-TXT-1 as form fields) decodes the picture under the upload rules (FR-AST-2/3, without storing anything), embeds it with the search model in the inference threadpool — `dinov2-large` unless `model` names another (FR-IMG-4) — and returns the nearest assets by cosine similarity of that model's embeddings. The query image is never persisted: it is written to a temporary file outside the media root and unlinked in a `finally`, so no asset row, no file under the media root and no trace of it outlives the request.
- **FR-IMG-2** `GET /api/v1/assets/{id}/similar?limit&offset&min_score` uses the asset's stored embedding of the search model as the query (no inference at all, so it answers on a build whose weights were never downloaded), excludes the asset itself from every page, and → 409 `not-indexed` when the asset has no vector for that model — corrected in change 11 from "when its DINOv2 job is not `done`", for the reason FR-TXT-3 records: a `reindex` puts the work back while the vector stays. An identifier no asset carries → 404. `tags_all`, `tags_any` and `meta.<key>` arrived with change 12: they narrow the neighbours, never the asking asset, so an asset may ask what looks like it among a set it does not itself belong to.
- **FR-IMG-2a** The exclusion happens inside the vector query, above the index scan and before the offset is applied: removing the asset after a page was cut would shift every later page by the row it removed. It costs one of the candidates the index may produce, so this search's `limit + offset` ≤ 998 — one page-depth less than FR-FLT-4 allows the others — and a deeper page is refused rather than answered from a search that could not reach it (change 11, design decision 5).
- **FR-IMG-3** Item and envelope shapes are those of FR-TXT-3/4, with `model` naming the model that ranked them and `query_truncated` always `false` — a picture is never cut short. `scan_limited` means what it means everywhere (FR-FLT-6).
- **FR-IMG-4** Every search MAY take `model=<key>` to select another embedder that supports the query modality (a CLIP image→image search, a future style model); an unsupported combination → 422 naming what that model can be asked, and a key this build does not run → 503 naming it. `app/domain.py` holds the one table of what each model takes, so the refusal costs no model load. Cross-model comparison is impossible by construction: the query is embedded by the model whose stored vectors are searched.
...
- **FR-FLT-2** `tags_all` (every listed tag present, `@>`) and `tags_any` (at least one, `&&`) accept comma-separated tags normalised as in FR-TAG-1; both MAY be combined. Filters are applied in the SQL of the vector query (a `WHERE` on the joined `assets` row, inside the window the index answers), never by post-filtering a candidate list the database already chose, so a narrow filter still returns `limit` items when they exist. Refinements from change 12, where this was implemented and measured (`docs/how-to/benchmarks.md`): a narrowed query sets `hnsw.iterative_scan = strict_order`, without which the index's candidates are chosen once and the filter empties the page (measured: nothing at all for one asset in a hundred, while thirty matches sat past the candidate window); **which plan answers is the database's judgement** — the vector index, the distances of exactly the narrowed assets, or, on a corpus small enough for that to be cheapest, the distances of every vector of the model — and the service promises the meaning of the answer rather than the plan; only the index path is bounded, and a search that stopped at that bound rather than at the end of the ranking says so in `scan_limited` (FR-FLT-6). Index parameters themselves remain the index ADR's.
- **FR-FLT-3** `meta.<key>=<value>` filters by top-level string equality in `meta` (`meta @> '{"key": "value"}'`); at most 5 such filters; keys match `^[a-z0-9_]{1,64}$`. Because the caller invents the name, it is read from the raw query string — and, on `POST /search/image`, from the raw form fields — rather than declared, and each operation's description carries the convention. The same key given twice is refused rather than silently `AND`-ed into a request that can never match. Every narrowing this service will not apply — a value that is not a tag, a key outside the pattern, more than five conditions, a repeated key — is 422 `/errors/invalid-filter` naming the value, on all four surfaces, and nothing is searched (change 12).
- **FR-FLT-4** Pagination is `limit` (default 20, max 100) and `offset`. On search endpoints `limit + offset` ≤ 999 (one short of pgvector's `hnsw.ef_search` maximum, which is also the most candidates one scan yields — the last of them answers `has_more`; see FR-FLT-5, amended with this in change 8); on listings `offset` ≤ 10 000. Pages are consistent only while the index does not change; the README states this. `total` is deliberately absent: with an approximate index and a threshold, an exact count costs a full scan.
- **FR-FLT-5** Every search query sets `hnsw.ef_search` for its transaction to `max(HNSW_EF_SEARCH, limit + offset + 1)` (default 40, never above 1 000) so deep pages do not silently lose recall; a page with `limit + offset` > 999 → 422 `page-too-deep` — corrected in change 8 from `max(HNSW_EF_SEARCH, limit + offset)` and a bound of 1 000, which cannot both hold: one HNSW scan yields at most `ef_search` rows, and the row that answers `has_more` is one of them, so the page ends one short of that ceiling.
- **FR-FLT-6** A narrowed search says whether it reached what the answer needed. `scan_limited` is true when the scan produced fewer candidates than `offset + limit + 1` **and** more assets satisfying the narrowing exist than it reached — one bounded existence query, asked only in that case, carrying the search's model, its narrowing and the asset an asset's own search excludes. It is decided by what the search reached, never by the shape of the answer: a full page can be cut short (the row beyond it was never reached) and a page emptied by `min_score` is not (change 12). Only a narrowed search can set it.

### 2.8 CLI (CLI)

A `typer` application `python -m app.cli` (also `uv run semanticshelf`), sharing settings, repositories and services with the API.

- **FR-CLI-1** `index-folder <dir> [--recursive] [--tags a,b] [--meta '{"k":"v"}'] [--dry-run] [--no-index]` walks the folder, feeds every JPEG/PNG/WebP file through the upload pipeline (FR-AST-2…7 unchanged, `source = folder`), skips duplicates by SHA-256 with a counter, enqueues jobs, and prints a summary (`created`, `already stored`, `refused` with reasons, `skipped` with reasons). The walk reads only regular files inside the directory: no symbolic link is read, no symlinked directory entered, and what a file is is decided from the descriptor the walk opened, not from its path. `original_filename` holds the file's own name and `meta.source_path` holds its path relative to the imported directory — corrected in change 7, because FR-AST-5 reduces `original_filename` to a single name so that no path survives in it, and the two sentences could not both hold. After a successful run the command carries out the work it created, claiming only those assets' jobs, and reports what it could not finish (`indexed`, `still queued`, `failed`); it never waits out a retry's backoff. `--no-index` leaves the work queued instead; `INDEXING_RUNNER` selects between this runner and the worker process, which exists from change 13 — under `INDEXING_RUNNER=worker` the command imports and queues and carries nothing out, exactly as `--no-index` does, and the summary names whichever of the two decided it. The two cannot contradict each other: the flag is about this run, the setting about this deployment. The same rule governs `index missing`, the other command that finishes work it created.
- **FR-CLI-2** `semanticshelf worker [--once] [--batch N]` is the stage-3 runner of FR-IDX-2, with graceful shutdown on SIGTERM or SIGINT: it stops claiming, finishes the batch it holds, reports what it did and ends, releasing nothing — work it could not finish returns when its lease expires. A second signal ends it at once, by restoring that signal's default disposition and re-raising it, so a supervisor sees the status it expects; a forced end says only that it was forced, because collecting totals first is the delay the second signal exists to remove. `--once` takes a single batch and exits, which is what a scripted run and a test want.
- **FR-CLI-3** `demo-dataset download [--count 500] [--into .data/demo]` fetches a bounded sample of a permissively licensed public image dataset with a manifest, writes each image with a sidecar of tags derived from the dataset's own labels/keywords and provenance `meta` (FR-TAG-2), and prints the licence notice. The dataset is **COCO val2017**, chosen in change 8's successor `add-demo-dataset` over the other candidate (Unsplash Lite, whose dataset terms forbid publishing any portion of the data and so collide with NFR-DOC-1's screenshots): its manifest carries a licence per picture, so the licence check is a filter the command runs rather than a claim it prints. Only the four licences that permit reuse are taken — NoDerivs is refused because the service re-encodes a thumbnail, which is a derivative work — and the details are recorded in `docs/reference/demo-dataset.md`. Images are never committed; `.data/` is gitignored. `demo-dataset index` runs `index-folder` on the result.
...
- **FR-OPS-3** `GET /api/v1/stats` → asset count, counts per `(model, status)`, pending job age (oldest `created_at`), storage bytes — the recorded sizes of the stored originals added up, not a measurement of the media root, so thumbnails are outside the number (stated so in change 8) — the UI's Status page and a smoke check for the README.
...
- Pagination envelope: `{items, limit, offset, has_more}`; search envelopes add `model`, `scan_limited` and, for text, `query_truncated`.
...
- **NFR-SEC-5** Untrusted input is bounded before processing: `q` ≤ 256 chars after trimming, with the raw parameter bounded at 1024 so that padding cannot make the request unbounded (change 8: FR-TXT-2 bounds the trimmed query, so the raw bound cannot be the same number), tags ≤ 32 × 64 chars, `meta` ≤ 8 KB and depth ≤ 4, multipart body ≤ `MAX_UPLOAD_BYTES`. Nothing from a request is interpolated into SQL (parameters only) or shell.
...
- deletion: rows, embeddings, jobs and files gone; second delete 404; delete-then-job race → the running job's finish lands nowhere, leaving no vector and no job;
...
- search: cosine ordering on known vectors; `min_score`; `limit`/`offset`/`has_more`; tag and meta filters combined with the vector query, including a narrowing that matches one asset in three hundred filling a page and the scan's own bound reached on purpose and reported (`scan_limited`); model isolation (a `dinov2-large` vector never appears in a `clip-vit-l14` search even with a compatible dimension fixture); `ef_search` bound; `query_truncated`; `/similar` excludes itself and refuses a non-indexed asset;
...
| 14 | `tune-vector-indexes` | HNSW vs IVFFlat per model: build time, recall@10, p95, filtered queries; `ef_search` curve; `docs/how-to/benchmarks.md`; **ADR-002** | **medium** (raised at the proposal: the guard that keeps a published benchmark out of the service's tables is refactored and a second such command is added) | numbers and commands published; the chosen parameters applied by migration if they changed — measured: they did not change |
