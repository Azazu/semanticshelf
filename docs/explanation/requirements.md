...
- **FR-TXT-1** `GET /api/v1/search/text?q=<text>&limit&offset&min_score&tags_all&tags_any` embeds `q` with the search model's text tower — `clip-vit-l14` unless `model` names another (FR-IMG-4) — and returns the nearest assets by cosine similarity of that model's image embeddings.
...
- **FR-IMG-3** Item and envelope shapes are those of FR-TXT-3/4, with `model` naming the model that ranked them and `query_truncated` always `false` — a picture is never cut short.
